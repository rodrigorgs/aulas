<?php
	...
	$sql = $conn->prepare("INSERT INTO resposta (timestamp, apostila, nome, matricula, button_index, answers, testes_ok, testes_total) VALUES (NOW(), 'unica', 'Sicrano', '0988907654', 0, ?, 10, 10);");
	$sql->bind_param("s", $value);
	$result = $sql->execute();

	$value = "{\"answers\":[\"Beltrano\"]}";
	$sql = $conn->prepare("INSERT INTO resposta (timestamp, apostila, nome, matricula, button_index, answers, testes_ok, testes_total) VALUES (NOW(), 'unica', 'Beltrano', '0987654321', 0, ?, 10, 10);");
	$sql->bind_param("s", $value);
	$result = $sql->execute();

	$value = "{\"answers\":[\"Fulano\"]}";
	$sql = $conn->prepare("INSERT INTO resposta (timestamp, apostila, nome, matricula, button_index, answers, testes_ok, testes_total) VALUES (NOW(), 'unica', 'Fulano', '1234567890', 0, ?, 10, 10);");
	$sql->bind_param("s", $value);
	$result = $sql->execute();

	$value = "{\"answers\":[\"Sicrana\"]}";
	$sql = $conn->prepare("INSERT INTO resposta (timestamp, apostila, nome, matricula, button_index, answers, testes_ok, testes_total) VALUES (NOW(), 'unica', 'Sicrana', '1233214567', 0, ?, 10, 10);");
	$sql->bind_param("s", $value);
	$result = $sql->execute();

	$value = "{\"answers\":[\"Fulana\"]}";
	$sql = $conn->prepare("INSERT INTO resposta (timestamp, apostila, nome, matricula, button_index, answers, testes_ok, testes_total) VALUES (NOW(), 'unica', 'Fulana', '1238634712', 0, ?, 10, 10);");
	$sql->bind_param("s", $value);
	$result = $sql->execute();
