...
<div id="loginbar">
  <span class="loginbar-logout">
    ...
    <button onclick="proximaResposta(-1);" class="admin-only">&lt;</button>
    <button onclick="proximaResposta(1);" class="admin-only">&gt;</button>
    <a href="${URL_PREFIX}assignment-stats.php?url=${window.location.href}&token=${submissionSession.getToken()}">Stats</a>
  </span>
...
function carregaRespostasTotal(username) {
  ...
  console.log('respostas', username);
  postSubmit({ username, submission_type: 'batch' }, "get-answers.php")
  .done((data) => {
    if (submissionSession.isAdmin() || confirm('Isso vai substituir suas respostas atuais pelas respostas enviadas por último. Deseja continuar?')) {
      carregaRespostasDeArray(data);
...
function proximaResposta(delta) {
  $.get(URL_PREFIX + `assignment-stats2.php?url=${window.location.href}`,
      data => {
        ...
        if (usernames.length == 0) {
          ...
          carregaRespostasTotal(usernames[idx]);
        }
      });
}
