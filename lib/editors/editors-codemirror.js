...
function buildButtons(codeMirror) {
  ...
  if (document.triplePage == true) {
    oldStdinTextarea.remove();
    stdoutTextarea = $('.console');
    stdinTextarea = $('#entrada');
  } else {
    let stdinTextarea = $('<textarea style="height: 3.8em; vertical-align: top; width: 49%;" class="stdin" placeholder="Entrada"></textarea>');
    ...
    stdoutTextarea = $('<textarea style="height: 3.8em;  vertical-align: top; width: 49%;" disabled class="stdout" placeholder="Saída"></textarea>');
  ...
  if (document.triplePage == undefined) {
