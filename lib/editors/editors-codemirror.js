...
function buildButtons(codeMirror) {
  ...
  let buttonRun = $('<button style="display: block" class="go" x-text="$store.submission.username ? \'Salvar e rodar\' : \'Rodar\'">Rodar</button>');
  buttonRun.insertAfter(editor);
  codeMirror.buttonRun = buttonRun;

  // TODO: se tiver teste
  let buttonTest = $('<button style="display: block" class="go" x-text="$store.submission.username ? \'Salvar e avaliar\' : \'Avaliar\'">Avaliar</button>');
  // buttonTest.insertAfter(editor);
  // buttonDiv.append(buttonTest);
  buttonTest.insertAfter(buttonRun);
  ...
  let doRun = () => {
    ...
    stdoutTextarea.show().fadeToggle(250, 'swing', () => {
      ...
      const codeToRun = runTemplate.replaceAll('[[[code]]]', codeMirror.getValue());
