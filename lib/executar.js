(function(){
    ...
    console.log = function (message){
        if(typeof(message) == 'object') {
            ...
            if(message.indexOf("%c", 0) >= 0){
                if(message.indexOf("%c", 0) == 0){
                    message = message.slice(2);
                }
                else{
                    if(message.indexOf("%c", 0) == message.length - 2){
                        message = message.slice(0, message.indexOf("%c", 0))
                    }
                    else{
                        message = message.slice(0, message.indexOf("%c", 0)) + message.slice(message.indexOf("%c", 0) + 2);
                    }
                }
            }
...
function fullScreenConsolelog(){
    ...
    $('#rightcolumn').css('text-align', 'center');
    $('#expandeConsole').off('click');
    $('#expandeConsole').on('click', voltaConsolelogAoNormal);
    $('#expandeConsole').attr('value', 'Normal');
    //console.log("Imprimiu");
}

function voltaConsolelogAoNormal(){
    $('#consolelog').css('width', '33.33%');
    $('#consolelog').css('text-align', 'left');
    $('.exercicio'+pagina_atual).css('display', 'inline');
    $('#volta').attr('disabled', disabledVolta);
    $('#avanca').attr('disabled', disabledAvanca);
    $('#expandeConsole').off('click');
    $('#expandeConsole').on('click', fullScreenConsolelog);   
    $('#expandeConsole').attr('value', 'Expandir');
...
function fullScreenEditor(){
    ...
    $('#editor'+pagina_atual).css('width', '100%');
    $('#botaoEditor'+pagina_atual).off('click');
    $('#botaoEditor'+pagina_atual).on('click', voltaEditorAoNormal);
    $('#botaoEditor'+pagina_atual).attr('value', 'Normal');
    //console.log("Foi");
}

function voltaEditorAoNormal(){
    $('#editor'+pagina_atual).css('width', '33.33%');
    $('#exercicio'+pagina_atual).css('display', 'inline');
    ...
    $('#avanca').attr('disabled', disabledAvanca);
    $('#botaoEditor'+pagina_atual).off('click');
    $('#botaoEditor'+pagina_atual).on('click', fullScreenEditor);   
    $('#botaoEditor'+pagina_atual).attr('value', 'Expandir');
...
function preparaLayout(){
    ...
    if($("#exercicio1").hasClass("temEditor")){
        $("#consolelog").css('display', 'inline');
	$("#input").css('display', 'inline');
        $('#botaoEditor1').on('click', fullScreenEditor);
    }
    else{
        $("#consolelog").css('display', 'none');
	$("#input").css('display', 'none');
    ...
    for(var i = 2; i <= num_paginas_atual; i++){
        ...
        if($("#exercicio"+i).hasClass("temEditor")){
            $('#botaoEditor'+i).on('click', fullScreenEditor);
        }
    }
...
function aninhaConteudo(){
    for (var i = 0, len = items.length; i < len; i++) {
        ...
        if(items[i].nodeName == "H2"){
            ...
            if(elementosDaDiv.length > 0) {
                for(var k = 0, elen = elementosDaDiv.length; k < elen; k++){
                    if(elementosDaDiv[k].nodeName == "INPUT" && elementosDaDiv[k].className == "editorFull"){
                    ...
                    if(elementosDaDiv[k].nodeName == "DIV" && elementosDaDiv[k].className == "CodeMirror cm-s-default"){
                        ...
                        k++;
                        while(!(elementosDaDiv[k].nodeName == "INPUT" && elementosDaDiv[k].className == "editorFull")){
                            divCodeMirror.append(elementosDaDiv[k]);
                            k++;
                        }
                        k = k - 1;    
        ...
        if(items[i].nodeName == "BUTTON" && items[i].className == "go"){
            //console.log(items[i].nodeName + " " + items[i].className);
            items[i].className += (" exercicio"+num_paginas_atual);
            elementosDaDiv.push(items[i]);
            botaoEditor = '<input type = "button" class = "editorFull" id = "botaoEditor'+num_paginas_atual+'" value = "Expandir"/>';
            items[i].insertAdjacentHTML('afterbegin', botaoEditor);     
            //console.log("Adicionou um botao");       
        }
	else{
            if(items[i].nodeName == "P" || items[i].nodeName == "IMG" || items[i].nodeName == "UL" || items[i].nodeName == "LI" || items[i].nodeName == "BUTTON" 
               ...
               || items[i].nodeName == "TEXT" || items[i].nodeName == "LINE" || items[i].nodeName == "POLYLINE" || items[i].nodeName == "POLYGON"
                || items[i].nodeName == "H3" || items[i].nodeName == "H4" || items[i].nodeName == "H5" || items[i].nodeName == "H6" || (items[i].nodeName == "PRE" && items[i].className == "highlight")){
                //console.log(items[i].nodeName + " " + items[i].id);
                elementosDaDiv.push(items[i]);
            }
        }
        ...
        if(items[i].nodeName == "DIV" && items[i].id == "consolelog" || items[i].nodeName == "FOOTER"){
            ...
            for(var k = 0, elen = elementosDaDiv.length; k < elen; k++){
                if(elementosDaDiv[k].nodeName == "DIV" && elementosDaDiv[k].className == "CodeMirror cm-s-default"){
                    ...
                    if(elementosDaDiv[k].nodeName == "INPUT" && elementosDaDiv[k].className == "editorFull"){
                        //console.log("Chegou no botão esperado");
                        divCodeMirror.append(elementosDaDiv[k]);
                    }
                    else{
                        //console.log(items[i].id);
